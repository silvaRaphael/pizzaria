 ...
 * @returns {void}
 */
function CustomConfirmationAlert(
	{
		title = 'Tem certeza dessa escolha?',
		buttonLabel = 'Sim, confirmar!',
		successMessage = 'Confirmado com sucesso!',
	},
	onConfirmed = () => {},
	onDone = () => {},
) {
	...
	swalWithBootstrapButtons
		.fire({
			html:
				'<div class="mt-3"><lord-icon src="https://cdn.lordicon.com/tdrtiskw.json" trigger="loop" colors="primary:#f06548,secondary:#f7b84b" style="width:100px;height:100px"></lord-icon><div class="mt-4 pt-2 fs-15 mx-5"><h4>' +
				title +
				'</h4></div></div>',
			reverseButtons: true,
			showCancelButton: true,
...
function buscarCEP(button, tab) {
	const cep = document
		.querySelector(`${tab} [name="cep"]`)
		.value.trim()
		.replace(/\D/g, '');

	if (cep.length != 8) return;

	const carregar_dados_plano_request = new ApiRequest(
...
function loadCities(stateId, selector, cityId) {
	const requestLoadCities = new ApiRequest(
		`/api/cities/${stateId}`,
		'GET',
		selector,
	);

	requestLoadCities.getField(
		'city_id',
	...
	requestLoadCities.onSuccess((response) => {
		response.forEach((item) => {
			requestLoadCities.getField(
				'city_id',
			).innerHTML += `<option value="${item.id}">${item.city}</option>`;
		});
 ...
 * @returns {Promise}
 */
async function sendRequest(url = '/', method = 'GET', body = {}, token = '') {
	try {
		let options = {
...
class ApiRequest {
	 ...
	 * @param {string?} selector
	 */
	constructor(url, method, selector, token) {
		this.url = url;
		this.method = method;
	...
	clearFields() {
		[
			...Object.values(document.querySelectorAll(`${this.selector} input`)),
			...Object.values(document.querySelectorAll(`${this.selector} select`)),
		].forEach((item) => {
	...
	setFields(fields) {
		Object.entries(fields).forEach((item) => {
			document.querySelector(`${this.selector} [name="${item[0]}"]`).value =
				item[1];
		});
	}
	...
	validate() {
		const errors = [];
		Object.values(
			document.querySelectorAll(`${this.selector} [required]`),
		).forEach((item) => {
			item.style.borderColor =
				!item.value || item.dataset.hasError ? 'red' : '';
	...
	buttonLoading() {
		if (this.button) {
			this.button.innerHTML =
				'<span class="spinner-border text-warning mx-auto" role="status" style="width: 1rem; height: 1rem;"></span>';
	...
	async send() {
		try {
			...
			const success =
				response.status === 204 ? response : await response.json();
